...
function chooseMusic(){
    $('input').click() //pass the element we want to click on, in this case 'input' control for file
    console.log($('input').attr('name'))
}

function musicSelected(){
    let files = $('input').get(0).files
    console.log($('input').get(0).name)
    console.log(files)

    for(let i = 0; i<files.length; i++){
        let {path} = files[i]
        mm.parseFile(path, {native:true}).then(metadata =>{ //we pass the metadata to the function
            ...
            $('#table-body').append(songRow) //jquery reference to the table
